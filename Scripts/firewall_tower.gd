...
@export var base_wait_time: float = 0.4
@export var min_wait_time: float = 0.05
@export var ramp_step: float = 0.05
...
var current_tracked_target: Node2D = null
...
func _ready() -> void:
	...
	shoot_timer.wait_time = base_wait_time
...
func _process(_delta: float) -> void:
	...
	if target != null:
		laser_beam.show()
		laser_beam.set_point_position(0, spawn_point.position)
		laser_beam.set_point_position(1, to_local(target.global_position))
	else:
		laser_beam.hide()
...
func _on_shoot_timer_timeout() -> void:
	...
	if target != current_tracked_target:
		current_tracked_target = target
		shoot_timer.wait_time = base_wait_time
	
	if target != null:
		shoot(target)
		shoot_timer.wait_time = max(min_wait_time, shoot_timer.wait_time - ramp_step)
